...
```bash
gsd-sdk query phase.list-learnings --window "$GRADUATION_WINDOW"
```
...
`gsd-sdk query state.load` and extract the `graduation_backlog` section (if present). Format:
...
`gsd-sdk query state.add-graduation-backlog` with payload:
```yaml
- cluster_id: "{sha256}"
  status: "deferred"
