...
3. **Read depth scales with context window** -- check `context_window` via `gsd-sdk query config-get context_window`:
...
**How to check:** Run `gsd-sdk query config-get context_window`. If the field is absent, treat as 200k (conservative default).
