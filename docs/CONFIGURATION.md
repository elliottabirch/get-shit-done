...
| `storage.adapter` | enum | `markdown`, `beads` | `markdown` | Storage backend selection (Phase 8 DIST-01). Default (`markdown`) is byte-identical to upstream GSD. Set `beads` with `gsd-beads` installed to route all `.planning/` state through a bd store. Omitting the key is equivalent to `markdown`. Throws `BeadsAdapterUnavailable` with install hint if `beads` is set but `gsd-beads` is not installed. |
