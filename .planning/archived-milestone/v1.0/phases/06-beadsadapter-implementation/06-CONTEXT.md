...
| **4** | 4 | `bd comments add --label` unsupported (v1.0.3 silent-drop / v1.0.4 hard-reject exit 1) | Labels silently dropped on v1.0.3; on v1.0.4 bd exits 1 with `Error: unknown flag: --label` (Plan 06-03 §5.1 reverified) | Use `--author gsd:event:<type>` exclusively for high-freq append event types; discipline unchanged across versions | 06-06 (recordStateAppend) |
