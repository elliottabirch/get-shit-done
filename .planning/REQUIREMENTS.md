...
- [ ] **READS-01**: All ~40 SDK read queries call `adapter.*` instead of `node:fs` directly (`progressJson`, `roadmapAnalyze`, `stateJson`, `findPhase`, `phasesList`, `phasePlanIndex`, `summaryExtract`, etc.)
...
- [ ] **READS-03**: Skill frontmatter `<context>`-block `@.planning/...` references audited; each leak is either rewritten through the adapter or documented as out-of-scope for skill activation
