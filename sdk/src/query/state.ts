...
import { stateExtractField, planningRelativePath, escapeRegex } from './helpers.js';
 ...
 */
export async function buildStateFrontmatter(adapter: StorageAdapter, bodyContent: string, projectDir: string, workstream?: string): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    for (const dir of phaseDirs) {
      const dirRefs = await adapter.listCollection(`${phasesRel}/${dir}`);
      const files = dirRefs.map(r => r.name);
