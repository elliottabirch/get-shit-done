...
// ─── ship.pr_body_sections validation (#3391) ────────────────────────────
...
export const configNewProject: QueryHandler = async (args, projectDir, workstream) => {
  ...
  // Validate ship.pr_body_sections if provided in userChoices
  if (userChoices.ship) {
    const ship = config.ship as Record<string, unknown>;
    if (ship.pr_body_sections !== undefined && (ship.pr_body_sections as unknown[]).length > 0) {
