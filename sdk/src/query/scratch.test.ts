...
describe('scratch — discuss.checkpoint.* + discuss.questions.* (PRIMITIVES-09)', () => {
  ...
  it('discuss.checkpoint.put/get/delete roundtrip at .planning/phases/05-foo/05-DISCUSS-CHECKPOINT.json', async () => {
    ...
    const putRes = await discussCheckpointPut(adapter, [phaseDir, phaseNum, body], tmpDir);
    expect((putRes.data as { written: string }).written).toContain('phases/05-foo/05-DISCUSS-CHECKPOINT.json');

    const getRes = await discussCheckpointGet(adapter, [phaseDir, phaseNum], tmpDir);
    ...
    await discussCheckpointDelete(adapter, [phaseDir, phaseNum], tmpDir);
    const afterDelete = await discussCheckpointGet(adapter, [phaseDir, phaseNum], tmpDir);
    expect((afterDelete.data as { found: boolean }).found).toBe(false);
  });

  it('discuss.questions roundtrip with json format', async () => {
    await discussQuestionsPut(adapter, ['05-foo', '05', 'json', '{"q":"x"}'], tmpDir);
    const res = await discussQuestionsGet(adapter, ['05-foo', '05', 'json'], tmpDir);
  ...
  it('discuss.questions roundtrip with html format', async () => {
    await discussQuestionsPut(adapter, ['05-foo', '05', 'html', '<p>q</p>'], tmpDir);
    const res = await discussQuestionsGet(adapter, ['05-foo', '05', 'html'], tmpDir);
    expect((res.data as { content: string }).content).toBe('<p>q</p>');
  });

  it('delete on nonexistent scratch file is a no-op (no throw)', async () => {
    await expect(discussCheckpointDelete(adapter, ['99-nonexistent', '99'], tmpDir)).resolves.toBeDefined();
  });

  it('rejects phaseDir with ".." (path traversal guard)', async () => {
    await expect(
      discussCheckpointPut(adapter, ['../../etc', '05', 'body'], tmpDir),
    ).rejects.toThrow(/\.\./);
  });

  it('rejects invalid format arg on questions', async () => {
    await expect(
      discussQuestionsPut(adapter, ['05-foo', '05', 'yaml', 'body'], tmpDir),
